...
describe('SimilarProducts', () => {
  const fakeProducts = [makeFakeProduct(), makeFakeProduct(),makeFakeProduct(), makeFakeProduct() ];
  ...
  it('renders the next page of products when next button is clicked', () => {
    render(withHistory(<SimilarProducts similarProducts={fakeProducts} />));

    userEvent.click(screen.getByLabelText('Следующая страница'));
    const visibleProducts = screen.getAllByTestId('product-card-div').filter((product) => product.classList.contains('is-active'));
    expect(visibleProducts.length).toBeLessThanOrEqual(3);
  });

  it('renders the prev page of products when prev button is clicked', () => {
    render(withHistory(<SimilarProducts similarProducts={fakeProducts} />));
